...
module.exports.index = async (req, res) => {
    ...
    const { search } = req.query; //Filter listing based on country
    
    const cityListings = search?allListings.filter(listing => listing.country.toLowerCase() === search.toLowerCase()) : allListings;
    if(!cityListings.length){
        req.flash("error", `No Listing exists for ${search}`);
        res.redirect("/listings");
    } 
    console.log(cityListings);
    res.render("listings/index.ejs", {allListings : cityListings, search});
